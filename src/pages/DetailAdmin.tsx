...
const DetailAdmin: FC<DetailAdminType> = ({ match }) => {
  const admin = useSelector((state: stateType) =>
    selectAdminById(state, match.params.id as any)
  );
  ...
  const deleteAdmin = async () => {
    setloading(true);
    try {
      const res = await fetchWithToken(`${API_URL}/api/user/delete/admin`, {
        id: match.params.id,
      });
  ...
  return (
    <SecondLayout
      header={<SecondaryHeader title={`detail admin`} />}
      title="Detail Admin"
    >
      <Alert
        ...
        <Card>
          ...
          <div className="flex flex-col gap-7">
            <div className="flex flex-col gap-1">
              ...
              <span className="text-sm">{admin.nama}</span>
            </div>
            <div className="flex flex-col gap-1">
              <span className="font-xl font-extrabold">Alamat</span>
              <span className="text-sm break-all">{admin.alamat}</span>
            </div>
            <div className="flex flex-col gap-1">
          ...
          <div className="w-full flex justify-center items-center gap-2 flex-col">
            <IonButton
              fill="clear"
              className="w-4/5"
              routerLink={`/tabs/admin/edit-admin/${match.params.id}/true`}
